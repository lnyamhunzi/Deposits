...
var PremiumStatus = /* @__PURE__ */ ((PremiumStatus2) => {
  PremiumStatus2["CALCULATED"] = "CALCULATED";
  PremiumStatus2["INVOICED"] = "INVOICED";
  PremiumStatus2["PAID"] = "PAID";
  PremiumStatus2["OVERDUE"] = "OVERDUE";
  PremiumStatus2["CANCELLED"] = "CANCELLED";
...
// server/penalty-levying-service.ts
import { eq, and, gte, lte } from "drizzle-orm";
import { v4 as uuidv4 } from "uuid";
var PenaltyLevyingService = class {
  db;
  constructor(db) {
    this.db = db;
  }
  async checkAndApplyPenalties() {
    const overdueInvoices = await this.db.query.invoices.findMany({
      where: and(
        ...
        eq(invoices.status, PremiumStatus.PENDING)
    ...
    for (const invoice of overdueInvoices) {
      const existingPenalty = await this.db.query.premiumPenalties.findFirst({
        where: and(
          eq(premiumPenalties.invoiceId, invoice.id),
          gte(
            premiumPenalties.createdAt,
      ...
      await this.db.insert(premiumPenalties).values(penalty);
...
//# sourceMappingURL=penalty-levying-service.js.map
